...
pthread_mutex_t sys_shutdown_control_mutex;
...
static bool is_shutdown_process_started() {
    w_mutex_lock(&sys_shutdown_control_mutex);
    bool ret_val = shutdown_process_started;
    w_mutex_unlock(&sys_shutdown_control_mutex);
    return ret_val;
}
...
void* wm_sys_main(wm_sys_t *sys) {
    ...
    w_mutex_init(&sys_shutdown_control_mutex, NULL);
...
void wm_sys_stop(__attribute__((unused))wm_sys_t *data) {
    ...
    w_mutex_destroy(&sys_shutdown_control_mutex);
