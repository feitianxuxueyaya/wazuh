...
cJSON* local_add(const char *id,
    ...
    if (id && (index = OS_IsAllowedID(&keys, id), index >= 0)) {
        if(OS_SUCCESS == w_auth_replace_agent(keys.keyentries[index], key_hash, force_options, &str_result)) {
            minfo("Duplicate ID '%s'. %s", id, str_result);
        } else {
            mwarn("Duplicate ID '%s' (name '%s'), rejecting enrollment. %s", id, keys.keyentries[index]->name, str_result);
            ierror = EDUPID;
            goto fail;
        }
    }
    ...
    if (strcmp(ip, "any")) {
        ...
        if(OS_SUCCESS == w_auth_replace_agent(keys.keyentries[index], key_hash, force_options, &str_result)) {
            ...
            mwarn("Duplicate IP '%s' (id '%s'), rejecting enrollment. %s", ip, keys.keyentries[index]->id, str_result);
            ierror = EDUPIP;
            goto fail;
        }
        }
    ...
    if (index = OS_IsAllowedName(&keys, name), index >= 0) {
        if(OS_SUCCESS == w_auth_replace_agent(keys.keyentries[index], key_hash, force_options, &str_result)) {
            ...
            mwarn("Duplicate name '%s' (id '%s'), rejecting enrollment. %s", name, keys.keyentries[index]->id, str_result);
            ierror = EDUPNAME;
            goto fail;
        }
    }
